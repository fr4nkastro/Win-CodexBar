...
fn parse_pi_assistant_entry(value: &Value, target: PiMappedProvider) -> Option<PiEntry> {
    ...
    let cost = match mapped {
        ...
        PiMappedProvider::Claude => {
            ...
            #[allow(clippy::cast_possible_truncation, reason = "token counts clamped to i32::MAX above")]
    ...
    for key in keys {
        if let Some(v) = usage.get(*key) {
            ...
            if let Some(n) = v.as_f64() {
                ...
                #[allow(clippy::cast_possible_truncation, reason = "token counts are whole numbers; fractional part is rounding noise")]
