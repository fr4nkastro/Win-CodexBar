...
fn window_minutes(start: Option<DateTime<Utc>>, end: Option<DateTime<Utc>>) -> Option<u32> {
    ...
    if minutes > 0 {
        #[allow(clippy::cast_possible_truncation, reason = "guarded by `minutes > 0`; a window longer than ~8 million years is impossible")]
        Some(minutes as u32)
    } else {
        None
    }
...
fn resets_at(
    ...
    #[allow(clippy::cast_possible_truncation, reason = "epoch seconds truncated to whole seconds by design")]
...
fn window_type_from_duration(start: Option<DateTime<Utc>>, end: Option<DateTime<Utc>>) -> String {
    ...
    if (23.0..=25.0).contains(&duration_hours) {
        ...
        #[allow(clippy::cast_possible_truncation, reason = "label only; duration already filtered into the 1..23 hours bucket")]
...
/// should be skipped.
#[allow(clippy::too_many_arguments, reason = "signature mirrors the flat upstream makeServiceUsage payload fields one-to-one")]
fn make_remains_row(
    service_type: &str,
    window_type_override: Option<&str>,
    total: Option<i64>,
