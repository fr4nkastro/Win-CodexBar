...
fn format_credit_number(value: f64) -> String {
    ...
    if (rounded - rounded.round()).abs() < 0.001 {
        #[allow(clippy::cast_possible_truncation, reason = "value just rounded and verified integral within 0.001, so the i64 cast loses nothing")]
...
fn epoch_seconds_or_millis(number: f64) -> Option<DateTime<Utc>> {
    ...
    #[allow(clippy::cast_possible_truncation, reason = "epoch seconds far exceed i64 range yield None from from_timestamp anyway; truncation only for absurd inputs that fail validation next")]
...
mod tests {
    ...
    fn hostile_cache_inputs_are_rejected() {
        ...
        #[allow(clippy::cast_possible_truncation, reason = "MAX_CACHE_BYTES is 1 MiB; +1 stays far inside usize on any supported target")]
