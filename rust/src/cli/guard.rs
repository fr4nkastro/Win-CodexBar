...
pub async fn run(args: GuardArgs) -> i32 {
    ...
    let web_timeout = if timeout_secs > 0.0 {
        ...
        #[expect(clippy::cast_possible_truncation, reason = "timeout validated to 0..=86400, so the cast cannot truncate")]
...
fn guard_percent_string(value: f64) -> String {
    ...
    if (value - rounded).abs() < 0.05 {
        ...
        #[expect(clippy::cast_possible_truncation, reason = "display-only percentage label")]
