...
impl CodexWorkspacesIndex {
        ...
        F: FnMut(Progress),
    {
        ...
        #[allow(clippy::cast_possible_truncation, reason = "progress UI field is u32; a session file count above 4 billion is not realistic")]
        ...
        for (idx, path) in files.iter().enumerate() {
            ...
            if idx == 0 || (idx + 1) % 25 == 0 || idx + 1 == files.len() {
                #[allow(clippy::cast_possible_truncation, reason = "index within the same files list whose len fits u32")]
